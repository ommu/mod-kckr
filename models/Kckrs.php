...
class Kckrs extends \app\components\ActiveRecord
{
	...
	public function init()
	{
		...
		$this->templateColumns['thanksUserDisplayname'] = [
			...
			'visible' => !Yii::$app->request->get('thanksUser') ? true : false,
		];
		$this->templateColumns['medias'] = [
			'attribute' => 'medias',
			'value' => function($model, $key, $index, $column) {
				$medias = $model->getMedias('count');
				return Html::a($medias, ['publisher/media/manage', 'kckr' => $model->primaryKey, 'publish' => 1], ['title' => Yii::t('app', '{count} karya', ['count' => $medias]), 'data-pjax' => 0]);
			},
			'filter' => false,
			'contentOptions' => ['class' => 'text-center'],
			'format' => 'raw',
