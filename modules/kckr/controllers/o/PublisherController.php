...
class PublisherController extends Controller
{
	...
	public function actionRunAction() {
		...
		if(count($id) > 0) {
			...
			if($actions == 'publish') {
				KckrPublisher::model()->updateAll(array(
					'publish' => 1,
				),$criteria);
				...
				KckrPublisher::model()->deleteAll($criteria);
			}
		}
	...
	{
		$model = KckrPublisher::model()->findByPk($id);
		if($model===null)
			throw new CHttpException(404, Yii::t('phrase', 'The requested page does not exist.'));
		return $model;
	}
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='kckr-publisher-form') {
